...
class Robertson():
    def __init__(self,Zij,B,weight):
        ...
        self.weight = weight
        self.t = B
        self.E = np.zeros((self.Zij.shape[0],self.Zij.shape[2]),dtype=np.float64)
        self.G = np.array([np.exp(np.arange(0, 1, 1 / 256)),np.exp(np.arange(0, 1, 1 / 256)),np.exp(np.arange(0, 1, 1 / 256))],dtype=np.float64).reshape(256,3)
        print(self.G.shape)
    def getE(self):
        # print(self.G.shape)
    ...
    def getG(self):
        ...
        for c in range(3):
            ...
            for i in range(256):
                ...
                # w = self.weight[z[idx]]
                top = np.sum(E[idx[0]]*self.t[idx[1]])
                down = len(idx[0])
                if down >0:
                    self.G[i,c] = top/down
       
    def solve(self,epochs=30):
...
class Debvec():
    def __init__(self,Zij,B,Weight,N,P):
